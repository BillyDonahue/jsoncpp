...
// exact copy of Reader, renamed to char_reader_detail::Reader
class Reader {
public:
  typedef char Char;
...
private:
  ...
  void operator=(Reader const&); // no impl
...
// complete copy of Read impl, for Reader
...
bool Reader::readStringSingleQuote() {
  Char c = 0;
  while (current_ != end_) {
    c = getNextChar();
    if (c == '\\')
...
class CharReader : public Json::CharReader {
  bool const collectComments_;
  Reader reader_;

public:
  CharReader(bool collectComments, Features const& features)
...
CharReader* CharReaderBuilder::newCharReader() const {
  ...
  char_reader_detail::Features features = char_reader_detail::Features::all();
  ...
  return new char_reader_detail::CharReader(collectComments, features);
}
